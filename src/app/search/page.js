...
const search = () => {
    ...
    const handleSearch = (e) => {
        let newuser = user.filter((user) => (user.username).toLowerCase().includes((search.toLowerCase())))
  ...
  return (
   <div>
    ...
    <div className='w-auto h-auto flex flex-col items-center space-y-2'>
        ...
        <input type='text' placeholder='Search' onChange={(e) => setSearch(e.target.value)} value={search} className='w-full h-full outline-none rounded-md border-none bg-slate-700 text-white p-2'/>
            <BiSearch className='text-white cursor-pointer text-[39px] rounded-md hover:bg-slate-800 p-2' onClick={handleSearch}/>
        </div>
        <div className='w-[900px] h-[auto] flex flex-wrap gap-3 justify-center items-center ml-auto mr-auto'>
            {
               ...
               user && user.map((item) => (
                    <div className='p-2 bg-gray-900' key={item.userId}>
                        <Image src={item.avatar} width={200} height={200} alt={item.username}/>
